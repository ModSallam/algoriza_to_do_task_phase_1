...
class AppCubit extends Cubit<AppStates> {
  ...
  void createDatabase() {
    openDatabase(
      ...
      onOpen: (database) {
        ...
        debugPrint('data received From Database successfully');
      },
    ).then((value) {
      database = value;
  ...
  void updateTaskStatus(Map model, s) {
    ...
    database
        ...
        .then((value) {
      debugPrint('${model['title']} is updated successfully');
      emit(AppUpdateTaskStatusSuccessfulState());
      getDataFromDataBase(database);
    }).catchError((error) {
      debugPrint('Error when update ${model['title']} ${error.toString()}');
